...
class MaleZombie extends SpriteAnimationComponent
  ...
  Future<void> _loadAnimations() async {
    ...
    //Running Animation
    final spriteSheet = await gameRef.images.load(ZombieImages.run);
    ...
    final idleImage = SpriteSheet(
      image: await gameRef.images.load(ZombieImages.idle),
      srcSize: spriteSize,
    );
    _standingAnimation =
        idleImage.createAnimation(row: 0, stepTime: _animationSpeed, to: 1);
    final deathSpriteSheet = await gameRef.images.load(ZombieImages.death);
    //death Animation
